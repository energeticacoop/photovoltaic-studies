...
function processEconomicFlows() {
  ...
  // Get tariffs and related values
  const tariff = getValue("tariff")
  const nationalHolidays = getColumn("nationalHolidays").map((e) => new Date(e))
  const tariffPeriods = dates.map((date) =>
    getTariffPeriod(date, tariff, nationalHolidays)
  )
  ...
  const monthlyBillComplete = monthlyBillPV.map((monthlyCost, index) => {
    compensableSurplusMeans[index] =
      monthlyCompensation[index] / (energeticaTariffCompensation * TAXES)
  ...
  const monthlyConvertedToSoles = monthlyBillComplete.map(
    (monthlyCost, index) => monthlyCost - monthlyBillLimitless[index]
  )
  setColumn("monthlyConvertedToSoles", monthlyConvertedToSoles)
  const monthlySolesInput = monthlyConvertedToSoles.map(
    (e) => e * fluxCoefficient
  )
...
/**
 * Processes all flows (energy and economic).
 * @returns {void}
 */
