...
function getSatelliteImages() {
  ...
  getMaps: for (let zoom = minZoom; zoom < maxZoom + 1; zoom++) {
    ...
    const roundedMetersPer100Px = roundToPrecision(metersPer100Px, scalePrecision);

    // Download image from Google Static Map API
    const currentFile = downloadMapImage(coordinates, zoom, size, scale, mapType, roundedMetersPer100Px, latitude, longitude, destinationFolder);
...
function calculateScale(latitude, zoom, scale) {
  return (((156543.03392 * Math.cos((latitude * Math.PI) / 180)) / Math.pow(2, zoom)) * 100) / scale;
}
 ...
 */
function downloadMapImage(coordinates, zoom, size, scale, mapType, scaleValue, latitude, longitude, destinationFolder) {
  ...
  return downloadFile(staticMapURL, currentFilename, destinationFolder).file;
}