  ...
  budget.forEach((element, index) => {
    ...
    switch (element.type) {
      case "title":
        ...
        row.getCell(0).editAsText().replaceText("title", element["Concepto título"])
        break
      case "item":
        row = budgetTable.insertTableRow(index, itemRow.copy())
        row.getCell(0).editAsText().replaceText("Concept", element["Concepto ítem"]).replaceText("Description", element["Descripción"])
